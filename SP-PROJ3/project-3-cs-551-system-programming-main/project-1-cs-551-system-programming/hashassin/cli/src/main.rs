...
enum Commands {
    ...
    GenRainbowTable {
        #[arg(long = "in-file")]
        in_file: String, // Input file containing seed passwords

        #[arg(long)]
        out_file: String, // Output file for the rainbow table

        #[arg(long, default_value_t = String::from("md5"))]
        algorithm: String, // Hashing algorithm to use

        #[arg(long, default_value_t = 5)]
        num_links: usize, // Number of links per chain

        #[arg(long, default_value_t = 1)]
        threads: usize, // Number of threads to use
    },

    // Dump contents of a rainbow table
    DumpRainbowTable {
        #[arg(long = "in-file")]
        in_file: String, // Input file containing the rainbow table
    },
    ...
    Crack {
        ...
        #[arg(long = "hashes")]
        hashes: String, // Input file containing hashes to crack

        #[arg(long)]
        out_file: Option<String>, // Optional output file for cracked passwords
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Match and execute the appropriate subcommand
    match &cli.command {
        ...
        Commands::GenRainbowTable {
            in_file,   // Input file containing seed passwords
            out_file,  // Output file for the rainbow table
            algorithm, // Hashing algorithm to use
            num_links, // Number of links per chain
            threads,   // Number of threads to use
        } => {
            info!(
                "Generating rainbow table from {} with {} links per chain using {} threads",
                in_file, num_links, threads
            );
            gen_rainbow_table(in_file, out_file, algorithm, *num_links, *threads)?;
        }

        // Subcommand to dump the contents of a rainbow table
        Commands::DumpRainbowTable { in_file } => {
            dump_rainbow_table(in_file)?; // Input file containing the rainbow table
        }

        // Subcommand to crack hashes using a rainbow table
        Commands::Crack {
            in_file,  // Input file containing the rainbow table
            hashes,   // Input file containing hashes to crack
            out_file, // Optional output file for cracked passwords
            threads,  // Number of threads to use
        } => {
            info!(
                "Cracking hashes using rainbow table from '{}' and hashes from '{}' with {} threads",
                in_file, hashes, threads
            );
            crack(in_file, hashes, out_file.as_deref(), *threads)?;
        }
    }
